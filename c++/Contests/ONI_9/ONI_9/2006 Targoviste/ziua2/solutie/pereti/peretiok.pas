...
begin
  a[x,y]:=a[x,y]+16;
  for i:=1 to 4 do
     if (a[x,y] and d[i]<>d[i]) and (a[x+dx[i],y+dy[i]]<16) then
                  fill(x+dx[i],y+dy[i]);
end;
...
for i:=1 to n do
         begin
         if a[0,i]<>24 then inc(nr);
         if a[m+1,i]<>18 then inc(nr);
         end;
     for i:=1 to m do
         begin
          if a[i,0]<>17 then inc(nr);
          if a[i,n+1]<>20 then inc (nr);
         end;
numara:=nr;
end;
