//Problema parc
//Autor : Zoltan Szabo
//Implementare alternativa : A. Panaete
//Complexitate O( N log N +M log M ) ( de la sortare ca prtea a doua e liniara )
#include<cstdio>
#include<algorithm>
#include<cmath>
#define MOD 31313
...
void read()
{
    ...
    int aux;
    scanf("%d",&m);m*=2;
    X[++R]=0;for(;m;m--){scanf("%d",&aux);X[++R]=aux;}X[++R]=XP;L=1;
    scanf("%d",&m);m*=2;
    Y[++U]=0;for(;m;m--){scanf("%d",&aux);Y[++U]=aux;}Y[++U]=YP;D=1;
}
void solve()
{
