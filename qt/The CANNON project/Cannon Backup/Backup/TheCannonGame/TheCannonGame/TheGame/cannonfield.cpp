...
void CannonField::paintEvent(QPaintEvent *e)
{
    ...
    if(started)
    {
        ...
        ChangeXY();
      //  qDebug() << "target: " << mapToGlobal(QPoint(targetCx,targetCy));
        //qDebug() << "targety: " << targetCy;
       // qDebug() << "shot: " << mapToGlobal(QPoint(shotx, shoty));
        //qDebug() << "shoty: " << shoty;

        qDebug() << '\n';
    }

    if((int)abs(shotx - targetCx) < Eps && shoty <= targetCy && shoty >= targetCy - targetsize)
    {
    ...
    if((int)abs(shoty - targetCy + targetsize) < Eps && shotx >= targetCx && shotx <= targetCx + targetsize/2 && !targetHit)
    {
        qDebug() << "ok";
        emit TargetWasHit();
...
void CannonField::AddExtraSpace()
{
    ...
    spaces[spacescnt]->setParent(this);
    ...
    spaces[spacescnt]->activateWindow();
    this->setFocus();

    spacescnt++;
}

void CannonField::ChangeMode()
{
    if(isBox == true)
    {
        isBox = false;
